  ...
  return (
    ...
    >
      ...
      >
        ...
        <nav
          className={cn(
            "absolute right-0 top-0 h-full w-full max-w-xs bg-background shadow-2xl sm:max-w-sm",
        ...
        >
          {mainNavigation.map((item) =>
            item.children ? (
              <div key={item.label} className="border-b border-border/50 py-3">
                ...
                <div className="mt-2 space-y-1">
                  {item.children.map((child) => (
                    <Link
                      key={child.label}
                      href={child.href}
                      className="block py-2 text-base font-medium text-foreground transition-colors hover:text-blue-600"
                      onClick={() => setMobileOpen(false)}
                    >
                      {child.label}
                    </Link>
              ...
              <Link
                key={item.label}
                href={item.href}
                className="border-b border-border/50 py-4 text-base font-medium text-foreground transition-colors hover:text-blue-600"
                onClick={() => setMobileOpen(false)}
              >
                {item.label}
              </Link>
